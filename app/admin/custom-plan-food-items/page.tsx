...
type SizeDraft = Omit<
  CustomPlanFoodSize,
  "foodItemId" | "displayOrder" | "isActive"
> & {
  displayOrder?: number;
  isActive?: boolean;
};
...
export default function CustomPlanFoodItemsPage() {
  const { data: plansData, isLoading: isLoadingPlans } =
    useGetMonthlyPlanAdminListQuery({ kind: "custom" });
  ...
  const { data: categoriesData, isLoading: isLoadingCategories } =
    useGetCustomPlanCategoriesAdminQuery(selectedPlanId, {
      skip: !selectedPlanId,
    });
  ...
  const [upsertFoodItem, { isLoading: isSaving }] =
    useUpsertCustomPlanFoodItemAdminMutation();
  const [deleteFoodItem, { isLoading: isDeleting }] =
    useDeleteCustomPlanFoodItemAdminMutation();
  const [reorderFoodItems, { isLoading: isReordering }] =
    useReorderCustomPlanFoodItemsAdminMutation();
  ...
  useEffect(() => {
    setForm((prev) => ({
      ...prev,
      categoryId: prev.id ? prev.categoryId : selectedCategoryId,
    }));
  }, [selectedCategoryId]);
    ...
    <section className="space-y-7">
      <div>
        ...
        <h2 className="mt-1 text-3xl font-semibold text-white">
          Make Your Plan Food Items
        </h2>
        <p className="mt-2 text-sm text-zinc-300">
          Upload and manage food cards, sizes, prices, and nutrition for each
          dynamic category.
        </p>
      </div>
          ...
          <div className="space-y-3">
            <div className="flex items-center justify-between gap-3">
              ...
              <button
                ...
                onClick={() =>
                  setForm((prev) => ({
                    ...prev,
                    sizes: [...prev.sizes, createSizeDraft()],
                  }))
                }
                className="rounded-xl bg-amber-300 px-3 py-2 text-xs font-semibold text-zinc-900"
              >
                + Add Size
              </button>
            </div>
            {form.sizes.map((size, index) => (
              <div
                ...
                className="grid gap-3 rounded-2xl border border-zinc-700/70 bg-zinc-950/45 p-4 md:grid-cols-2 xl:grid-cols-8"
              >
                <label className="space-y-1">
                  <span className="text-xs uppercase tracking-[0.12em] text-zinc-400">
                    Label
                ...
                <label className="space-y-1">
                  ...
                  <input
                    value={size.unit ?? ""}
                    onChange={(event) =>
                      updateSize(index, { unit: event.target.value })
                    }
                    placeholder="g/ml"
                    className="w-full rounded-xl border border-zinc-600 bg-zinc-900/70 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-amber-300"
                  />
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    value={size.price}
                    onChange={(event) =>
                      updateSize(index, { price: Number(event.target.value) })
                    }
                    className="w-full rounded-xl border border-zinc-600 bg-zinc-900/70 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-amber-300"
                  />
                </label>
                ...
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    value={size.protein}
                    onChange={(event) =>
                      updateSize(index, { protein: Number(event.target.value) })
                    }
                    className="w-full rounded-xl border border-zinc-600 bg-zinc-900/70 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-amber-300"
                  />
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    value={size.carbs}
                    onChange={(event) =>
                      updateSize(index, { carbs: Number(event.target.value) })
                    }
                    className="w-full rounded-xl border border-zinc-600 bg-zinc-900/70 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-amber-300"
                  />
                </label>
                <label className="space-y-1">
                  ...
                  <input
                    ...
                    value={size.fat}
                    onChange={(event) =>
                      updateSize(index, { fat: Number(event.target.value) })
                    }
                    className="w-full rounded-xl border border-zinc-600 bg-zinc-900/70 px-3 py-2 text-sm text-zinc-100 outline-none focus:border-amber-300"
                  />
                </label>
                <div className="flex items-end">
                  <button
                    ...
                    className="w-full rounded-xl border border-rose-400/40 bg-rose-500/10 px-3 py-2 text-sm text-rose-100 disabled:opacity-50"
                  >
                    Remove
                  </button>
                </div>
      ...
      (selectedPlanId && isLoading) ? (
        <LoadingState label="Loading custom plan food items..." />
      ) : null}
      {isError ? (
        <ErrorState label="Failed to load custom plan food items." />
      ) : null}
        ...
        <section className="admin-panel rounded-2xl p-5">
          ...
          <div className="mt-4 space-y-3">
            {items.map((item, index) => (
              <article
                key={item.id}
                className="rounded-2xl border border-zinc-700/70 bg-zinc-950/45 p-4"
              >
                <div className="flex flex-col gap-3 lg:flex-row lg:items-start lg:justify-between">
                  ...
                  <div className="flex flex-wrap gap-2">
                    <button
                      ...
                      className="rounded-lg border border-zinc-600 px-3 py-1.5 text-xs text-zinc-100 disabled:opacity-50"
                    >
                      Up
                    </button>
                    <button
                    ...
                    <button
                      ...
                      disabled={isDeleting}
                      className="rounded-lg border border-rose-400/40 bg-rose-500/10 px-3 py-1.5 text-xs text-rose-100 disabled:opacity-50"
                    >
                      Delete
                    </button>
                  </div>
            ...
            {!items.length ? (
              <p className="text-sm text-zinc-400">
                No food items found for this plan/category.
              </p>
            ) : null}
          </div>
        </section>
