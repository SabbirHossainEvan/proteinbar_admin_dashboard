...
type CustomPlanCategoryPayload = Omit<
  CustomPlanCategory,
  "id" | "slug" | "displayOrder"
> & { id?: string; slug?: string; displayOrder?: number };
type CustomPlanFoodItemPayload = Omit<
  CustomPlanFoodItem,
  "id" | "displayOrder" | "sizes"
> & {
  id?: string;
  displayOrder?: number;
  sizes: Array<
    Omit<CustomPlanFoodSize, "id" | "foodItemId" | "displayOrder"> & {
      id?: string;
      displayOrder?: number;
    }
  >;
...
const entities: MonthlyPlanEntities = {
  plans: {
    "plan-custom-core": {
      ...
      title: "Custom Plan",
      description:
        "Flexible monthly setup with configurable meals, days, snacks, and delivery rules.",
      ...
      content: {
        ...
        selectMealsText:
          "Pick meals by week/date. Changes apply instantly to your checkout.",
        checkoutText:
          "Review your selections and confirm your preferred delivery option.",
        customStepTwo: {
          categories: [],
          foodItems: [],
        },
    ...
    "plan-normal-lean": {
      ...
      description:
        "Pre-made balanced plan with weekly meal assignment for predictable operations.",
      image:
      ...
      content: {
        ...
        selectMealsText:
          "Pick from assigned meals for each date and meal type.",
        checkoutText:
          "Confirm your pickup/delivery mode and complete the subscription.",
      },
...
const buildWeekKeys = (startDate: string) =>
...
const customWeekTwo: WeekAssignment = {
  ...
  endDate: "2026-03-21",
  mealsByDate: weekTwoKeys.reduce<
    Record<string, WeekAssignment["mealsByDate"][string]>
  >((acc, day) => {
    acc[day] = [];
    return acc;
  }, {}),
...
const seededCustomPlanFoodItems: CustomPlanFoodItem[] = [
  ...
  },
  {
    ...
    name: "Avocado",
    imageUrl:
      "https://images.unsplash.com/photo-1519162808019-7de1683fa2ad?w=600",
...
const normalizeContent = (
  content?: MonthlyPlan["content"],
): NonNullable<MonthlyPlan["content"]> => ({
...
const cloneCustomPlanFoodItem = (
  item: CustomPlanFoodItem,
): CustomPlanFoodItem => ({
  ...item,
  sizes: item.sizes.map((size) => ({ ...size })),
});
...
const cloneDetails = (
  payload: MonthlyPlanDetailsPayload,
  ...
  rules: {
    ...
    deliveryOptionConfigs: payload.rules.deliveryOptionConfigs.map(
      (config) => ({ ...config }),
    ),
  },
  pricing: {
  ...
  mealLibrary: (payload.mealLibrary ?? asArray(entities.mealLibrary)).map(
    cloneMeal,
  ),
});
...
const ensureValidDetailPayload = (payload: MonthlyPlanDetailsPayload) => {
  ...
  if (
    ...
    !payload.rules.planTypeOptions.includes(payload.rules.defaults.planType)
  ) {
    throw new Error("Default plan type must exist in plan type options.");
  }
  if (
    payload.rules.deliveryDaysRule.allowedWeekDays.some(
      (day) => day < 0 || day > 6,
    )
  ) {
  ...
  if (
    payload.rules.defaults.deliveryDays.some(
      (day) => !payload.rules.deliveryDaysRule.allowedWeekDays.includes(day),
    )
  ) {
  ...
  payload.weekAssignments.forEach((week) => {
    if (week.startDate > week.endDate) {
      throw new Error(
        `Week ${week.weekIndex} start date must be before end date.`,
      );
    }
  ...
  payload.plan.content?.customStepTwo?.categories.forEach((category) => {
    if (!category.name.trim())
      throw new Error("Custom plan category name is required.");
    if (
      category.selectionMode === "single" &&
      category.maxSelect !== null &&
      category.maxSelect !== undefined &&
      category.maxSelect !== 1
    ) {
      throw new Error(
        `Category ${category.name} must have max select 1 when single-select.`,
      );
    }
    if (
      ...
      throw new Error(
        `Category ${category.name} has invalid min/max selection rules.`,
      );
    }
  });
  ...
  payload.plan.content?.customStepTwo?.foodItems.forEach((item) => {
    if (!item.name.trim())
      throw new Error("Custom plan food item name is required.");
    if (
      !payload.plan.content?.customStepTwo?.categories.some(
    ...
    if (!item.imageUrl.trim())
      throw new Error(`Food item ${item.name} requires an image URL.`);
    if (!item.sizes.length)
      throw new Error(`Food item ${item.name} requires at least one size.`);
    item.sizes.forEach((size) => {
      if (!size.label.trim())
        throw new Error(`Food item ${item.name} has a size without label.`);
      if (
        [size.price, size.calories, size.protein, size.carbs, size.fat].some(
          (value) => value < 0,
        )
      ) {
        throw new Error(
          `Food item ${item.name} has invalid negative size values.`,
        );
      }
    });
  });
...
export const monthlyPlanMockAdapter = {
  ...
  async listPlans(filters: ListFilters): Promise<MonthlyPlan[]> {
    ...
    const list = asArray(entities.plans).filter((plan) => {
      ...
      const matchSearch =
        !search ||
        `${plan.title} ${plan.description}`.toLowerCase().includes(search);
  ...
  async getPlanById(id: string): Promise<MonthlyPlanDetailsPayload | null> {
    ...
    return wait(
      cloneDetails({
        ...
        weekAssignments: plan.weekAssignmentIds
          .map((item) => entities.weekAssignments[item])
          .filter(Boolean),
        mealLibrary: asArray(entities.mealLibrary).sort((a, b) =>
  ...
  async upsertPlanDetails(
    payload: MonthlyPlanDetailsPayload,
  ): Promise<MonthlyPlanDetailsPayload> {
    ...
    if (normalizedPayload.plan.planKind === "custom") {
      ...
      normalizedPayload.plan.content?.customStepTwo?.categories.forEach(
        (category, index) => {
      ...
      normalizedPayload.plan.content?.customStepTwo?.foodItems.forEach(
        (item, index) => {
    ...
    entities.plans[plan.id].weekAssignmentIds =
      normalizedPayload.weekAssignments.map((item) => item.id);
    ...
    return wait(
      cloneDetails({
        ...
        weekAssignments: normalizedPayload.weekAssignments.map(
          (item) => entities.weekAssignments[item.id],
        ),
        mealLibrary: asArray(entities.mealLibrary).sort((a, b) =>
          a.name.localeCompare(b.name),
        ),
  ...
  async archivePlan(
    id: string,
  ): Promise<{ id: string; status: MonthlyPlan["status"] } | null> {
    const plan = entities.plans[id];
    if (!plan) return wait(null);
    entities.plans[id] = {
      ...plan,
  ...
  async listMealLibrary(): Promise<MealLibraryItem[]> {
    return wait(
      asArray(entities.mealLibrary).sort((a, b) =>
        a.name.localeCompare(b.name),
      ),
    );
  },
  ...
  async listCustomPlanCategories(
    planId: string,
  ): Promise<CustomPlanCategory[]> {
    return wait(getCustomPlanCategoriesForPlan(planId));
  },

  async upsertCustomPlanCategory(
    payload: CustomPlanCategoryPayload,
  ): Promise<CustomPlanCategory> {
    const existing = payload.id
      ? entities.customPlanCategories[payload.id]
      : null;
    if (!payload.name.trim()) throw new Error("Category name is required.");
    if (!payload.planId.trim())
      throw new Error("Plan is required for category.");

    const siblingCategories = asArray(entities.customPlanCategories).filter(
      (category) =>
        category.planId === payload.planId && category.id !== payload.id,
    );
    ...
    if (nextCategory.minSelect < 0)
      throw new Error("Min select cannot be negative.");
    if (
      (nextCategory.maxSelect ?? null) !== null &&
      nextCategory.minSelect > (nextCategory.maxSelect ?? 0)
    ) {
      throw new Error("Min select must be less than or equal to max select.");
    }

    entities.customPlanCategories[nextCategory.id] =
      cloneCustomPlanCategory(nextCategory);
    return wait(
      cloneCustomPlanCategory(entities.customPlanCategories[nextCategory.id]),
    );
  },
  ...
  async reorderCustomPlanCategories(
    planId: string,
    categoryIds: string[],
  ): Promise<CustomPlanCategory[]> {
    categoryIds.forEach((id, index) => {
      if (
        ...
        entities.customPlanCategories[id].planId === planId
      ) {
        entities.customPlanCategories[id] = {
          ...entities.customPlanCategories[id],
  ...
  async listCustomPlanFoodItems(
    planId: string,
    categoryId?: string,
  ): Promise<CustomPlanFoodItem[]> {
    const items = getCustomPlanFoodItemsForPlan(planId).filter(
      (item) => !categoryId || item.categoryId === categoryId,
    );
    return wait(items);
  },

  async upsertCustomPlanFoodItem(
    payload: CustomPlanFoodItemPayload,
  ): Promise<CustomPlanFoodItem> {
    const existing = payload.id
      ? entities.customPlanFoodItems[payload.id]
      : null;
    if (!payload.name.trim()) throw new Error("Food item name is required.");
    if (!payload.planId.trim())
      throw new Error("Plan is required for food item.");
    if (!payload.categoryId.trim())
      throw new Error("Category is required for food item.");
    if (!payload.imageUrl.trim())
      throw new Error("Image is required for food item.");
    if (!payload.sizes.length)
      throw new Error("At least one size is required.");
    if (!entities.customPlanCategories[payload.categoryId])
    ...
    entities.customPlanFoodItems[nextItem.id] = cloneCustomPlanFoodItem({
      ...nextItem,
      sizes: nextItem.sizes.map((size) => ({
        ...size,
        foodItemId: nextItem.id,
      })),
    });

    return wait(
      cloneCustomPlanFoodItem(entities.customPlanFoodItems[nextItem.id]),
    );
  },
  ...
  async reorderCustomPlanFoodItems(
    planId: string,
    categoryId: string,
    itemIds: string[],
  ): Promise<CustomPlanFoodItem[]> {
    itemIds.forEach((id, index) => {
      if (
        ...
        entities.customPlanFoodItems[id].planId === planId
      ) {
        entities.customPlanFoodItems[id] = {
          ...entities.customPlanFoodItems[id],
          categoryId,
    ...
    return wait(
      getCustomPlanFoodItemsForPlan(planId).filter(
  ...
  async listLocations(): Promise<LocationRecord[]> {
    return wait(
      asArray(entities.locations).sort((a, b) => a.name.localeCompare(b.name)),
    );
  },
  ...
  async updateSettings(
    patch: Partial<MonthlyPlanGlobalSettings>,
  ): Promise<MonthlyPlanGlobalSettings> {
